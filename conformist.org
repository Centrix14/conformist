...
Also library provides standard pattern system which can be created this way:
#+begin_src lisp
  (make-instance 'pattern-system)
#+end_src

As you can see it's just an instance of =pattern-system= with slots by default.
